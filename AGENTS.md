...
Cloudflare 可以作为候选平台，但不能盲目承诺 Cloudflare 足够。必须按需求验证。
...
```text
apps/ios/                 # iOS client
services/api/             # backend API, job worker, and agent runtime
...
当需求属于“多个 agent 多轮讨论、过程可展示、最终由裁判收敛，并通过 iOS/H5/Android/小程序等多端透出”时，默认使用本仓库沉淀的通用服务端模式：
...
- 客户端只消费产品层 `job / event / artifact`，不要直接理解 AgentScope session、team、worker 或 raw event。
- 服务端必须有 `Deliberation Job Service` 或等价业务层，负责输入快照、状态机、事件归一化、成本控制、失败恢复和最终 artifact。
- AgentScope 是当前第一候选 runtime adapter；自研轻量状态机和 CrewAI Flow 是 fallback。
- Claude Code CLI / Qoder CLI 只作为内部研发和离线评测工具，不直接接用户生产数据。
- 新项目复用该模式时，只替换输入快照、专家模板、轮次计划、裁判 prompt、safety policy、artifact schema 和客户端 renderer。

### Python Environment

凡涉及 Python 的服务端、AgentScope spike、脚本、测试或依赖安装，必须使用当前工作区内的虚拟环境：
...
- 不使用系统 Python 直接安装依赖。
- 不把依赖装到全局 site-packages。
- 不在仓库外创建项目虚拟环境，除非作者明确要求。
- 优先使用本项目 `.venv/bin/python`、`.venv/bin/pip` 或 `uv` 绑定该虚拟环境。
- 如果依赖要求特定 Python 版本，先在工作区 `.venv` 中创建匹配版本，再继续。
- `.venv/` 必须保持 git ignored。
...
- 每次本地验证通过一个可运行版本后，必须及时发布到远程服务器。
- 发布后至少验证远程 `/api/health`、关键新增接口或页面入口，确认远程版本和本地代码一致。
- 如果远程发布失败，不能声称版本已交付；要说明失败点、当前远程状态和下一步修复动作。
- 本地和远程存在差异时，优先先同步远程，再进入下一轮功能开发或真机验收。
...
5. 决定 iOS prototype 与服务端 prototype 的切分方式。
